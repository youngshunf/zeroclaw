...
pub type RouterSlot = Arc<std::sync::OnceLock<Arc<crate::TenantRouter>>>;
...
static GLOBAL_TENANT_ROUTER: std::sync::OnceLock<Arc<crate::TenantRouter>> =
    std::sync::OnceLock::new();

/// 注册全局 TenantRouter（由 channels/mod.rs 在 tenant_router 创建后调用）。
pub fn register_global_router(router: Arc<crate::TenantRouter>) {
...
pub fn tenant_security() -> Option<Arc<SecurityPolicy>> {
    zeroclaw_runtime::tools::get_active_security()
}
...
fn tenant_workspace(fallback: &Path) -> PathBuf {
    zeroclaw_runtime::tools::get_active_workspace().unwrap_or_else(|| fallback.to_path_buf())
}
...
impl Tool for HxSkillInstall {
    ...
    async fn execute(&self, args: Value) -> anyhow::Result<ToolResult> {
        ...
        // 6. Security audit
        match zeroclaw_runtime::skills::audit::audit_skill_directory(&dest_dir) {
            Ok(warnings) => {
