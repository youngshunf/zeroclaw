...
use crate::config::TenantHeartbeatConfig;
use crate::db::{TenantDb, TenantRecord, UserFilter};
...
impl TenantHeartbeatManager {
    ...
    async fn scan_and_execute(&self) -> Result<usize> {
        ...
        for user in users {
            ...
            let handle = tokio::spawn(async move {
                ...
                let tx_queue = crate::channel_registry::get_inbound_queue();
    ...
    fn best_delivery_channel<'a>(channels: &'a [crate::db::ChannelRecord]) -> &'a crate::db::ChannelRecord {
        if channels.is_empty() {
...
fn schedule_matches(expr: &str, now: DateTime<Utc>, window_minutes: u32) -> bool {
    match zeroclaw_runtime::cron::normalize_expression(expr) {
