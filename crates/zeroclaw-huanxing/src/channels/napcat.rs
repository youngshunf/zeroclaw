...
/// Sub-directory under the global workspace for caching downloaded NapCat media.
const NAPCAT_MEDIA_CACHE_DIR: &str = "napcat_media";
            ...
            .map(str::trim)
        {
            if !voice_path.is_empty() {
                ...
                if let Ok(bytes) = tokio::fs::read(clean_path).await {
                    let b64 = general_purpose::STANDARD.encode(&bytes);
                    tracing::info!(
                        "napcat voice: encoded {} bytes as base64 from {}",
                        bytes.len(),
                        clean_path
                    );
                    ...
                    if let Some(cq) = crate::voice::compose_napcat_voice_segment(trimmed)
                    {
                        parts.push(cq);
                    }
                }
...
impl NapcatChannel {
    ...
    pub fn from_config_with_workspace(
        ...
        let media_dir = workspace_dir.map(|ws| ws.join(NAPCAT_MEDIA_CACHE_DIR));
    ...
    fn http_client(&self) -> reqwest::Client {
        zeroclaw_config::schema::build_runtime_proxy_client("channel.napcat")
    }
