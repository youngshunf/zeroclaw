...
pub async fn extract_tenant_dir(
    ...
    config: &crate::config::HuanXingConfig,
) -> Option<String> {
...
async fn handle_audio_transcribe(
    ...
    match zeroclaw_channels::transcription::transcribe_audio(data, &file_name, &config.transcription)
        .await
    {
        Ok(text) => {
...
mod tests {
    ...
    use crate::config::HuanXingConfig;
    ...
    fn build_create_agent_params_uses_resolved_tenant_dir() {
        ...
        let config = crate::config::HuanXingConfig::default();
