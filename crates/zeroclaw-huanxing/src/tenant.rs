...
pub use zeroclaw_channels::orchestrator::ConversationHistoryMap;
...
pub struct TenantContext {
    ...
    pub heartbeat: zeroclaw_config::schema::HeartbeatConfig,

    /// Per-tenant cron config (from [cron] override or global).
    pub cron: zeroclaw_config::schema::CronConfig,
    ...
    pub knowledge_graph: Option<std::sync::Arc<zeroclaw_memory::knowledge_graph::KnowledgeGraph>>,
...
impl TenantContext {
    fn config_dir(global_config: &zeroclaw_config::schema::Config) -> PathBuf {
        global_config
            .config_path
            .parent()
    ...
    pub async fn load(
        ...
        let skills = zeroclaw_runtime::skills::load_skills_cascaded(
        ...
        let knowledge_graph: Option<
            std::sync::Arc<zeroclaw_memory::knowledge_graph::KnowledgeGraph>,
            ...
            && owner_dir == workspace_dir
        {
            if let Some(agents_dir) =
                crate::knowledge_cross::agents_dir_from_workspace(&workspace_dir)
            {
                let max_nodes = effective_knowledge_config.max_nodes;
                Some(std::sync::Arc::new(
                    tokio::task::spawn_blocking(move || {
                        crate::knowledge_cross::CrossWorkspaceKnowledgeIndex::discover(
                            &agents_dir,
                            max_nodes,
                        )
                    })
        ...
        let conversation_histories: ConversationHistoryMap =
            Arc::new(Mutex::new(lru::LruCache::new(
                std::num::NonZeroUsize::new(zeroclaw_channels::MAX_CONVERSATION_SENDERS).unwrap(),
            )));
...
fn create_session_backend(
    ...
    match channels_config.session_backend.as_str() {
        "sqlite" => {
            match zeroclaw_infra::session_sqlite::SqliteSessionBackend::new(workspace_dir) {
                Ok(b) => {
...
mod tests {
    ...
    async fn create_agent_uses_tenant_prompt_snapshot_instead_of_rereading_workspace() {
        ...
        assert!(
            matches!(
                &history[0],
                zeroclaw_providers::ConversationMessage::Chat(msg)
                    if msg.role == "system"
