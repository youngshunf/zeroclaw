...
struct AgentSession {
    ...
    session_backend: Option<std::sync::Arc<dyn zeroclaw_infra::session_backend::SessionBackend>>,
    ...
    ws_observer: Option<std::sync::Arc<dyn zeroclaw_runtime::observability::Observer>>,
...
pub async fn handle_ws_chat(
    ...
    if state.pairing.require_pairing() {
        ...
        // HuanXing Desktop integration: The local UI passes the Cloud JWT (starting with "eyJ")
        // instead of an OpenClaw pairing token. Since the sidecar is local and REST endpoints 
        // rely on this behavior, we allow it passing through when HuanXing mode is enabled.
        let is_hx_token = state.config.lock().huanxing.enabled && token.starts_with("eyJ");
        
        if !state.pairing.is_authenticated(token) && !is_hx_token {
            return (
    ...
    while let Some(msg) = receiver.next().await {
        ...
        match frame {
            ...
            InboundFrame::HistoryRequest {
                ...
                let messages_with_meta: Vec<zeroclaw_infra::session_backend::MessageWithMeta> = {
...
async fn resolve_tenant_context(
...
) -> anyhow::Result<Option<crate::TenantContext>> {
    ...
    let result = crate::TenantContext::load_by_agent_or_hasn(config, agent_name).await;
    match &result {
...
async fn init_agent_session(
    ...
    let mut agent = if let Some(ref tenant) = tenant_context {
        ...
        zeroclaw_runtime::agent::Agent::from_config(&config).await?
    };
    ...
    let ws_observer: Option<std::sync::Arc<dyn zeroclaw_runtime::observability::Observer>> = {
        let obs = std::sync::Arc::new(crate::ws_observer::WsObserver::new());
...
async fn process_chat_message(
    ...
    let (event_tx, mut event_rx) = tokio::sync::mpsc::channel::<zeroclaw_runtime::agent::TurnEvent>(64);
    ...
    loop {
        tokio::select! {
            ...
            event = event_rx.recv() => {
                match event {
                    Some(zeroclaw_runtime::agent::TurnEvent::Chunk { delta }) => {
                    ...
                    Some(zeroclaw_runtime::agent::TurnEvent::Thinking { delta }) => {
                    ...
                    Some(zeroclaw_runtime::agent::TurnEvent::ToolCall { name, args }) => {
                    ...
                    Some(zeroclaw_runtime::agent::TurnEvent::ToolResult { name, output }) => {
        ...
        if turn_result.is_some() && event_rx.is_empty() {
            ...
            while let Ok(event) = event_rx.try_recv() {
                match event {
                    ...
                    zeroclaw_runtime::agent::TurnEvent::Thinking { delta } => {
                        for line in delta.lines() {
                            let trimmed = line.trim();
                            if !trimmed.is_empty() {
                                progress_lines.push(trimmed.to_string());
