...
pub async fn run_sync(
    hub_dir: &PathBuf,
    sync_config: &crate::config::HubSyncConfig,
...
pub fn needs_sync(hub_dir: &PathBuf, sync_config: &crate::config::HubSyncConfig) -> bool {
    let registry_path = hub_dir.join("registry.json");
    if !registry_path.exists() {
        return true;
    }
...
/// 启动时检查并自动同步（异步后台任务）
pub fn auto_sync_on_startup(hub_dir: PathBuf, sync_config: crate::config::HubSyncConfig) {
...
/// 解析 hub 目录路径
fn resolve_hub_dir(config: &zeroclaw_config::schema::Config) -> PathBuf {
    config
        .huanxing
        .resolve_hub_dir()
