    ...
    let run_id = {
        ...
        match engine_guard.start_run(&name, event) {
            Ok(action) => match action {
                zeroclaw_runtime::sop::SopRunAction::ExecuteStep { run_id, .. } => run_id,
                zeroclaw_runtime::sop::SopRunAction::WaitApproval { run_id, .. } => run_id,
                zeroclaw_runtime::sop::SopRunAction::DeterministicStep { run_id, .. } => run_id,
                zeroclaw_runtime::sop::SopRunAction::CheckpointWait { run_id, .. } => run_id,
                zeroclaw_runtime::sop::SopRunAction::Completed { run_id, .. } => run_id,
                zeroclaw_runtime::sop::SopRunAction::Failed { run_id, .. } => run_id,
            },
            Err(e) => {
                return (
...
pub struct RunsListResponse {
    pub runs: Vec<zeroclaw_runtime::sop::SopRun>,
}
        ...
        || status_filter == Some("cancelled")
    {
        for run in engine_guard.finished_runs(None) {
            let status_str = match run.status {
                zeroclaw_runtime::sop::SopRunStatus::Completed => "completed",
                zeroclaw_runtime::sop::SopRunStatus::Failed => "failed",
                zeroclaw_runtime::sop::SopRunStatus::Cancelled => "cancelled",
