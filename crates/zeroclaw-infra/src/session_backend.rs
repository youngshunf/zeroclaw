...
/// 唤星扩展：带元数据的消息封装。
///
/// 供 `load_with_metadata` / `append_with_metadata` 使用，允许 session backend
/// 保存每条消息附带的不透明 JSON 元数据（典型用途：`{"progress_lines": [...]}`）。
#[derive(Debug, Clone)]
...
pub trait SessionBackend: Send + Sync {
    ...
    /// 唤星扩展：带 metadata 版的 load。默认实现从 `load()` 构造无 metadata
    /// 的结果；支持 metadata 的 backend 可覆写。
    fn load_with_metadata(&self, session_key: &str) -> Vec<MessageWithMeta> {
        self.load(session_key)
            .into_iter()
            .map(|message| MessageWithMeta {
                message,
                metadata: None,
            })
            .collect()
    ...
    /// 唤星扩展：带 metadata 版的 append。默认实现退化为 `append()`
    /// 并丢弃 metadata；支持 metadata 的 backend 可覆写。
