...
use crate::market_api::{get_download_info, download_bytes, unzip_buffer};
...
fn substitute_placeholders(
    ...
    content
        ...
        // In desktop mode, we just leave them empty because desktop uses `.huanxing/config.toml` global configs.
...
impl AgentFactory {
    ...
    pub async fn process_workspace(
        ...
        let process_file = |src_path: &Path, file_name: &str, dest_dir: &Path| -> Result<()> {
            if file_name.ends_with(".template") {
                ...
                std::fs::write(dest_dir.join(dest_name), substitute_placeholders(&content, params, def, &now))?;
            } else if file_name.ends_with(".md") || file_name == "template.yaml" {
                let content = std::fs::read_to_string(src_path)?;
                std::fs::write(dest_dir.join(file_name), substitute_placeholders(&content, params, def, &now))?;
            } else {
        ...
        if layer1.is_dir() {
            ...
            for scaffold in crate::scaffold::agent_scaffold() {
                ...
                std::fs::write(workspace.join(dest_name), substitute_placeholders(scaffold.content, params, def, &now))?;
            }
        }
        ...
        if template_dir.is_dir() {
            fn process_recursive(dir: &Path, dest: &Path, params: &CreateAgentParams, def: &TemplateDefinition, now: &str) -> Result<()> {
                for entry in std::fs::read_dir(dir)? {
                    ...
                    if path.is_file() {
                        ...
                        std::fs::write(dest.join(&name), substitute_placeholders(&content, params, def, now))?;
                    } else if path.is_dir() {
            ...
            if owner_dir.is_dir() {
                ...
                for entry in std::fs::read_dir(&owner_dir)? {
                    ...
                    if entry.path().is_file() {
                        ...
                        std::fs::write(target_path, substitute_placeholders(&content, params, def, &now))?;
                    }
                }
            }
        }
        ...
        if !owner_processed && used_embedded {
            ...
            for scaffold in crate::scaffold::owner_scaffold() {
                ...
                if !target_path.exists() {
                    std::fs::write(&target_path, substitute_placeholders(scaffold.content, params, def, &now))?;
                }
            }
        }
    ...
    pub async fn install_dependencies(
        ...
        let dest_subdir = target_dir.join("workspace").join(format!("{}s", resource_type));
    ...
    pub async fn install_from_market(
        ...
        self.process_workspace(&base_dir, &base_desktop_dir, tmpdir.path(), &target_dir, params, &def, progress).await?;

        // Download skills & sops
        if let Some(api) = &self.market_api_base {
            self.install_dependencies(api, "skill", &def.skills.exclusive, &target_dir, progress).await?;
            self.install_dependencies(api, "sop", &def.sops, &target_dir, progress).await?;
        }
    ...
    pub async fn create_local_agent(
        ...
        self.process_workspace(&base_dir, &base_desktop_dir, &template_dir, &target_dir, params, &def, progress).await?;
