...
pub async fn consolidate_turn(
        ...
        && !update.trim().is_empty()
    {
        ...
        // Fire-and-forget distillation to MEMORY.md when a workspace is provided.
...
/// Asynchronously rewrite MEMORY.md using the LLM provider based on existing
/// MEMORY.md and the new core update.
async fn distill_core_memory_to_markdown(
    ...
    match provider
        .chat_with_system(Some(system_prompt), &prompt, model, 0.1)
        .await
    {
        Ok(new_content) => {
