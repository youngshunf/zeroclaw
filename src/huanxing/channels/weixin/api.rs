...
fn build_headers(token: &str, body_len: usize) -> HeaderMap {
    ...
    // X-WECHAT-UIN
    let rand_uint32 = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().subsec_nanos();
...
pub async fn get_upload_url(channel: &WeixinChannel, mut req: GetUploadUrlReq) -> Result<GetUploadUrlResp> {
...
pub async fn get_updates_loop(
    ...
    loop {
        ...
        let resp_result = client.post(&url)
            .headers(headers)
            .body(body_str)
            .timeout(req_timeout)
            .send()
            ...
            Ok(resp) => {
                ...
                if let Some(msgs) = get_updates_resp.msgs {
                    for wmsg in msgs {
                        ...
                        if sender_id.is_empty() { continue; }
                            ...
                            if item.item_type == Some(MESSAGE_ITEM_TYPE_TEXT) {
                                if let Some(txt_item) = item.text_item {
                                    if let Some(t) = txt_item.text {
                                        ...
                                        let msg_id = item.msg_id.unwrap_or_else(|| Uuid::new_v4().to_string());
                                        ...
                                        if let Err(e) = tx.send(cm).await {
                                            tracing::error!("Failed to route weixin message to bus: {}", e);
                                        }
                                    }
                                }
                            }
