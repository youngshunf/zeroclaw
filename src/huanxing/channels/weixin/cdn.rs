...
pub async fn upload_media_to_cdn(
    ...
    tracing::debug!(
        "Weixin CDN upload preparing: rawsize={} filesize={} md5={} filekey={}",
        rawsize,
        filesize,
        rawfilemd5,
        filekey
    ...
    let target_url = upload_resp.upload_full_url.unwrap_or_else(|| {
    ...
    tracing::debug!(
        "Weixin CDN POST url={} ciphertextSize={}",
        target_url,
        ciphertext.len()
    );
    ...
    let download_encrypted_query_param =
        download_param.ok_or_else(|| anyhow!("Missing x-encrypted-param in CDN response"))?;
