...
use crate::huanxing::hasn_chat_db::HasnChatDb;
use crate::huanxing::hasn_connector::HasnAgentSession;
use crate::huanxing::agent_bridge::global_bridge;
...
impl HasnAgentBridge {
    ...
    pub async fn classify_and_annotate(
        ...
        let is_owner = if let Ok(tenant_db) = crate::huanxing::db::TenantDb::open(&db_path) {
    ...
    pub async fn inject_and_stream(
        ...
        if let Some(ref backend) = session.session_backend {
            let user_msg = crate::providers::ChatMessage::user(&injection_prompt);
        ...
        tokio::spawn(async move {
            ...
            match agent_lock.turn_streamed(&injection_prompt, event_tx_ch).await {
                Ok(full_reply) => {
                    ...
                    let record = crate::huanxing::hasn_chat_db::ChatMessageRecord {
                        id: 0,
