...
mod tests {
    ...
    fn duckduckgo_status_hint_for_403_mentions_provider_switch() {
        let hint = HxWebSearchTool::duckduckgo_status_hint(StatusCode::FORBIDDEN);
    ...
    fn duckduckgo_status_hint_for_500_is_empty() {
        assert!(
            HxWebSearchTool::duckduckgo_status_hint(StatusCode::INTERNAL_SERVER_ERROR).is_empty()
        );
    }
    ...
    fn provider_chain_uses_primary_plus_fallbacks_and_dedupes() {
        let tool = HxWebSearchTool::new_with_options(
            test_security(),
            "duckduckgo".to_string(),
            None,
            None,
