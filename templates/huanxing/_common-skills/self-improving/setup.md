...
```markdown
Use `memory_store` for factual continuity (events, context, decisions).
...
If in doubt, store factual history via `memory_store`, and store reusable performance lessons in `~/self-improving/` (tentative until human validation).
```
...
```markdown
- When someone says "remember this" → if it's factual context/event, use `memory_store`; if it's a correction, preference, workflow/style choice, or performance lesson, log it in `~/self-improving/`
