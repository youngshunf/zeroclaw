...
export default function Cron() {
  ...
  const handleAdd = async () => {
    if (!formSchedule.trim() || !formCommand.trim()) { setFormError(t('cron_extra.required')); return; }
    ...
    catch (err: unknown) { setFormError(err instanceof Error ? err.message : t('cron_extra.add_failed')); }
    finally { setSubmitting(false); }
  };

  const handleDelete = async (id: string) => {
    try { await deleteCronJob(id); setJobs(prev => prev.filter(j => j.id !== id)); }
    catch (err: unknown) { setError(err instanceof Error ? err.message : t('cron_extra.del_failed')); }
    finally { setConfirmDelete(null); }
  };

  if (error && jobs.length === 0) return <div className="hx-error-card"><h2>{t('cron.error') || 'Error'}</h2><p>{error}</p></div>;
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <h2 style={{ fontSize: 15, fontWeight: 600, color: 'var(--hx-text-primary)' }}>{t('cron_extra.title')} ({jobs.length})</h2>
        </div>
        <button onClick={() => setShowForm(true)} style={{
          ...
          <Plus size={14} />{t('cron_extra.add_btn')}
        </button>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ background: 'var(--hx-bg-main)', border: '1px solid var(--hx-border)', borderRadius: 16, padding: 24, width: '100%', maxWidth: 440, margin: '0 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <h3 style={{ fontSize: 16, fontWeight: 600, color: 'var(--hx-text-primary)' }}>{t('cron_extra.add_title')}</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: 'var(--hx-text-secondary)', marginBottom: 4 }}>{t('cron_extra.name_opt')}</label>
                <input type="text" value={formName} onChange={e => setFormName(e.target.value)} placeholder={t('cron_extra.name_pl')} style={inputStyle} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: 'var(--hx-text-secondary)', marginBottom: 4 }}>{t('cron_extra.schedule')} <span style={{ color: '#DC2626' }}>*</span></label>
                <input type="text" value={formSchedule} onChange={e => setFormSchedule(e.target.value)} placeholder={t('cron_extra.schedule_pl')} style={inputStyle} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 500, color: 'var(--hx-text-secondary)', marginBottom: 4 }}>{t('cron_extra.command')} <span style={{ color: '#DC2626' }}>*</span></label>
                <input type="text" value={formCommand} onChange={e => setFormCommand(e.target.value)} placeholder={t('cron_extra.command_pl')} style={inputStyle} />
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20 }}>
              <button onClick={() => { setShowForm(false); setFormError(null); }} style={{ padding: '8px 16px', fontSize: 13, fontWeight: 500, color: 'var(--hx-text-secondary)', border: '1px solid var(--hx-border)', borderRadius: 8, background: 'transparent', cursor: 'pointer' }}>{t('cron_extra.cancel')}</button>
              <button onClick={handleAdd} disabled={submitting} style={{ padding: '8px 16px', fontSize: 13, fontWeight: 500, color: 'white', background: 'var(--hx-purple)', borderRadius: 8, border: 'none', cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.5 : 1 }}>{submitting ? t('cron_extra.adding') : t('cron_extra.add')}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {jobs.length === 0 ? (
        <div className="hx-card" style={{ textAlign: 'center', padding: '40px 20px' }}>
          ...
          <p style={{ color: 'var(--hx-text-tertiary)', fontSize: 13 }}>{t('cron_extra.no_jobs')}</p>
        </div>
      ) : (
          ...
          <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--hx-border)' }}>
                {[
                  { key: 'id', label: t('cron_extra.th_id') },
                  { key: 'name', label: t('cron_extra.th_name') },
                  { key: 'cmd', label: t('cron_extra.th_cmd') },
                  { key: 'next', label: t('cron_extra.th_next') },
                  { key: 'status', label: t('cron_extra.th_status') },
                  { key: 'enabled', label: t('cron_extra.th_enabled') },
                  { key: 'actions', label: t('cron_extra.th_actions'), align: 'right' as const },
                ].map(h => (
              ...
              {jobs.map(job => (
                <tr key={job.id} style={{ borderBottom: '1px solid var(--hx-border-light)' }}>
                  ...
                  <td style={{ padding: '10px 16px' }}>
                    ...
                    }}>{job.enabled ? t('cron_extra.enabled') : t('cron_extra.disabled')}</span>
                  </td>
                  <td style={{ padding: '10px 16px', textAlign: 'right' }}>
                    {confirmDelete === job.id ? (
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: 12, color: '#DC2626' }}>{t('cron_extra.del_confirm')}</span>
                        <button onClick={() => handleDelete(job.id)} style={{ fontSize: 12, fontWeight: 500, color: '#DC2626', background: 'none', border: 'none', cursor: 'pointer' }}>{t('cron_extra.yes')}</button>
                        <button onClick={() => setConfirmDelete(null)} style={{ fontSize: 12, fontWeight: 500, color: 'var(--hx-text-tertiary)', background: 'none', border: 'none', cursor: 'pointer' }}>{t('cron_extra.no')}</button>
                      </span>
                    ) : (
