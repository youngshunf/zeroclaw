    ...
    <div>
      <div className="hx-metrics-grid">
        <div className="hx-metric-card">
          <div className="hx-metric-head"><DollarSign /><span>{t('cost_extra.session') || '会话账单'}</span></div>
          <div className="hx-metric-value">{formatCNY(cost.session_cost_usd)}</div>
        </div>
        <div className="hx-metric-card">
          <div className="hx-metric-head"><TrendingUp /><span>{t('cost_extra.daily') || '今日消耗'}</span></div>
          <div className="hx-metric-value">{formatCNY(cost.daily_cost_usd)}</div>
        </div>
        <div className="hx-metric-card">
          <div className="hx-metric-head"><Layers /><span>{t('cost_extra.monthly') || '本月消耗'}</span></div>
          <div className="hx-metric-value">{formatCNY(cost.monthly_cost_usd)}</div>
        </div>
        <div className="hx-metric-card">
          <div className="hx-metric-head"><Hash /><span>{t('cost_extra.requests') || '请求次数'}</span></div>
      ...
      <div className="hx-card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--hx-border)' }}>
          <h3 style={{ fontSize: 15, fontWeight: 600, color: 'var(--hx-text-primary)' }}>{t('cost_extra.model_breakdown') || '模型消耗占比'}</h3>
        </div>
        {models.length === 0 ? (
          <div style={{ padding: 32, textAlign: 'center', color: 'var(--hx-text-tertiary)', fontSize: 13 }}>
            {t('cost_extra.no_data') || '暂无数据'}
          </div>
        ) : (
            ...
            <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--hx-border)' }}>
                  <th style={{ textAlign: 'left', padding: '10px 20px', fontWeight: 500, color: 'var(--hx-text-tertiary)' }}>{t('cost_extra.th_model') || '模型'}</th>
                  <th style={{ textAlign: 'right', padding: '10px 20px', fontWeight: 500, color: 'var(--hx-text-tertiary)' }}>{t('cost_extra.th_cost') || '费用'}</th>
                  <th style={{ textAlign: 'right', padding: '10px 20px', fontWeight: 500, color: 'var(--hx-text-tertiary)' }}>{t('cost_extra.th_tokens') || 'Tokens'}</th>
                  <th style={{ textAlign: 'right', padding: '10px 20px', fontWeight: 500, color: 'var(--hx-text-tertiary)' }}>{t('cost_extra.th_requests') || '请求'}</th>
                  <th style={{ textAlign: 'left', padding: '10px 20px', fontWeight: 500, color: 'var(--hx-text-tertiary)' }}>{t('cost_extra.th_share') || '占比'}</th>
                </tr>
              </thead>
              <tbody>
